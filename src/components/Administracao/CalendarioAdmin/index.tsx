...
const CalendarioPagein = ({ calendario }: { calendario: CalendarioType[] }) => {
  return (
    <div
        ...
        {calendario.map((item, index) => (
          ...
          >
            <div className="w-16 h-16 bg-(--bg-input) border-2 border-(--border-color) rounded-[10px] flex flex-col items-center justify-center shrink-0">
              <span className="text-[24px] font-bold text-(--text-primary) leading-7">
                {item.dia}
              </span>
              <span className="text-[11px] font-semibold text-(--text-muted) uppercase leading-3.5">
                {item.mes}
              </span>
            </div>
...
export default CalendarioPagein;
