...
const NaoEncontrada = ({ logado, cargo }: AuthPaginas) => {
  ...
  return (
    <FundoBolhas>
      ...
      <h1 className="bg-linear-to-br from-blue-500 to-purple-500 bg-clip-text text-transparent text-[80px] md:text-[120px] font-extrabold leading-none mb-4 animate-glow">
        404
      </h1>
      ...
      <div className="bg-(--bg-input) border border-(--border-color) rounded-xl p-6 text-left">
        ...
        <ul className="list-none p-0">
          <li className="text-[14px] text-(--text-primary) px-2 py-0 pl-6 relative">
            <span className="text-(--primary-color) font-semibold">→</span>{" "}
            Verifique se o endereço foi digitado corretamente
          </li>
          <li className="text-[14px] text-(--text-primary) px-2 py-0 pl-6 relative">
            <span className="text-(--primary-color) font-semibold">→</span> Use
            a barra de busca para encontrar o que precisa
          </li>
          <li className="text-[14px] text-(--text-primary) px-2 py-0 pl-6 relative">
            <span className="text-(--primary-color) font-semibold">→</span>{" "}
